...
//this function breaks down the XMLs based on lines. 
//it detects the start of lines and sees if its a tag. If it is, it would add it to the list of spans that represent the tags.
//this process allows us to highlight the spans later on.
...
//this function strips the  end and begining tag to look at the tag name in between
...
function createTagElements(node) {
    ...
    tag_list.push({
        ...
        id: parseInt((tag_textbox.id.split('-').pop()))
    });
...
//this function adds spans to tags that are suppose to be highlighted.
//its structured based on IDs. the ID of the span matches the ID of the value box and the tag associated with it.
...
//function to cycle through the Value Boxes and looks for a matching tag in the Output area at the right.
function UpdateOutPutWithChanges(i) {
    ...
    } else if (string[0] != "" && string[0] != undefined && string[0] != null) {
        createWarning("can't recognize string " + string + "Make sure all fields have the begin/end tags <>", "XML_inputText_Div", " alert-danger");
    }
}
//a simple function that handles which warning we are going to give based on the detected issue
function HandleWarningsAndErrors({
    length
...
//this function gives the Text Paragraph a dash type indentation based on where the XML tag is in terms of depth
...
//this function takes tex and a warning Type. it uses bootstrap to create a Warning.
...
//this perevents the scroller from jumping when the page refreses.
history.scrollRestoration = "manual"
$(window).on('unload', function () {
    $(window).scrollTop(0);

});
//function to do some necissairy setup for elements.
...
$("#DownloadButton").click(function (e) {
    ...
    var newXmlStr = {
        data: s.serializeToString(xmlDoc_output)
    };

    //request to save the file to the database and then format it to look pretty and then send it back to download/.
    $.ajax({
        type: 'POST',
        url: '/download',
        ...
        success: function (data) {
           ...
           //we add the random just to avoid mixing the files names.
            ...
            DownLoadfile(`Upload-${RandomSnapShot}.xml`,data)
        }
    });
})
//this function takes a text string and a file name to create a single click automatic download link.
//the fucntion creates the link and clicks it and then removes it.
